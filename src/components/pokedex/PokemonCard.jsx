...
const PokemonCard = ({pokemonUrl}) => {
  ...
  return (
    <article className={`pokemonCard border-${pokemon?.types[0].type.name}`} onClick={handleClickPokemon}>
        ...
        <h3 className={`pokemonCard__name text-${pokemon?.types[0].type.name}`}>{pokemon?.name}</h3>
        ...
        <section className='pokemonCard__stats'>
            {
                ...
                <div className='pokemonCard__stat' key={stat.stat.url}>
                    ...
                    <h5 className={`pokemonCard__stat-value text-${pokemon?.types[0].type.name}`}>{stat.base_stat}</h5>
                </div>
                    ))
            }
        </section>
